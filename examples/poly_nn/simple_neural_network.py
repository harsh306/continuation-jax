...
class SimpleNeuralNetwork(AbstractProblem):
    def __init__(self):
        pass
        # self.inputs =  random.normal(random.PRNGKey(1), (1, 2 * 2))
        # self.outputs = np.ones(shape=(1, 1))
    ...
    def initial_values(self) -> Tuple:
        ...
        state_1, state_def = tree_flatten(state)
        state_0 = [i + 0.05 for i in state_1]
        state_2 = tree_unflatten(state_def, state_0)
        states = [state, state_2]
        del state_1, state_0

        bparam_1, bparam_def = tree_flatten(bparam)
        bparam_0 = [i + 0.05 for i in bparam_1]
        bparam_2 = tree_unflatten(bparam_def, bparam_0)
        bparams = [bparam, bparam_2]
